...
public class SpatialiteUtils {
    ...
    public final static String DB_NAME = "local_data_val_danno.sqlite";
    ...
    private final static String USER_RESULT_POS_LAT = "LAT";
    private final static String USER_RESULT_POS_LON = "LON";
    private final static String USER_RESULT_RADIUS = "RADIUS";
    ...
    public static boolean createNamesTable(final Database db){
        ...
        sb.append("CREATE TABLE '").append(NAMES_TABLE)
                ...
                .append("'").append( USER_RESULT_POS_LAT ).append( "' DOUBLE, " )
                .append("'").append( USER_RESULT_POS_LON ).append( "' DOUBLE, " )
                .append("'").append( USER_RESULT_RADIUS ).append( "' INTEGER, " )
    ...
    public static boolean insertIntoNamesTable(final Database db, final String createdTableName){
        ...
        sb.append("INSERT INTO ").append(NAMES_TABLE)
                ...
                .append(USER_RESULT_POS_LAT).append(",")
                .append(USER_RESULT_POS_LON).append(",")
                .append(USER_RESULT_RADIUS).append(",")
    ...
    /**
     * update the names table entry with elaboration data
     * @param db the db to update
     * @param elab the object containing the elaborations data
     * @return if this operation was successful, i.e. without exceptions
     */
    public static boolean updateNameTableWithElaboration(final Database db, final ElaborationResult elab){

        StringBuilder sb = new StringBuilder(200);
        sb.append("UPDATE ").append( NAMES_TABLE )
                .append(" SET ")
                .append(USER_RESULT_NAME).append(" = '").append(elab.getUserEditedName()).append("' , ")
                .append(USER_RESULT_POS_LAT).append(" = '").append(elab.getLocation().latitude).append("' , ")
                .append(USER_RESULT_POS_LON).append(" = '").append(elab.getLocation().longitude).append("' , ")
                .append(USER_RESULT_RADIUS).append(" = '").append(elab.getRadius()).append("' , ")
                .append(USER_RESULT_DESCRIPTION).append(" = '").append(elab.getUserEditedDescription())
                .append("' WHERE ").append(CREATED_TABLE_NAME).append(" = '").append(elab.getResultTableName()).append("';");
     ...
     * note that the description can be null
     * @param db the db to work on
     * @return a list of String[]
     */
    @Trace(category = MetricCategory.DATABASE)
    public static ArrayList<ElaborationResult> getSavedResults(final Database db){

        ArrayList<ElaborationResult> results = new ArrayList<>();
        StringBuilder sb = new StringBuilder(100);
        sb.append("SELECT ")
                .append(CREATED_TABLE_NAME).append(",")
                .append(USER_RESULT_POS_LAT).append(",")
                ...
                .append(USER_RESULT_RADIUS).append(",")
                .append(USER_RESULT_NAME).append(",")
                .append(USER_RESULT_DESCRIPTION)
        ...
        try{
            ...
            while (stmt.step()) {
                ...
                final String created = stmt.column_string(0);
                final Double lat = stmt.column_double(1);
                final Double lon = stmt.column_double(2);
                final Integer rad = stmt.column_int(3);
                final String name = stmt.column_string(4);
                final String desc = stmt.column_string(5);

                results.add(new ElaborationResult(name, desc, created, new GeoPoint(lat, lon), rad));
            }
    ...
    /**
     * checks if a title for a result is already used
     * @param db the db to work on
     * @param title the title for the new result
     * @return true if this title already exists, false otherwise
     */
    public static boolean checkIfUserTitleExists(final Database db, final String title){

        StringBuilder sb = new StringBuilder(100);
        sb.append("SELECT ").append(USER_RESULT_NAME)
                .append(" FROM ").append(NAMES_TABLE)
                .append(" WHERE ").append(USER_RESULT_NAME).append("='").append(title).append("';");
    ...
    public static Pair<Boolean,String> saveResult(final Database db, final CRSFeatureCollection result, final String geomType) {
        ...
        try {
            ...
            for (Feature feature : result.features) {
                ...
                arc_id = feature.properties.containsKey(ID_GEO_ARCO) ? (int) Math.rint((Double) feature.properties.get(ID_GEO_ARCO)) : 0; //is parsed by GSON as double, why ?
                risk1 = feature.properties.containsKey(RISCHIO1) ? (Double) feature.properties.get(RISCHIO1) : 0;
                risk2 = feature.properties.containsKey(RISCHIO2) ? (Double) feature.properties.get(RISCHIO2) : 0;
                ...
                label_soc = feature.properties.containsKey(LABEL_SOCIALE) ? (String) feature.properties.get(LABEL_SOCIALE) : "";
                label_amb = feature.properties.containsKey(LABEL_AMBIENTALE) ? (String) feature.properties.get(LABEL_AMBIENTALE) : "";

                label_tot = feature.properties.containsKey(LABEL_TOTALE) ? (String) feature.properties.get(LABEL_TOTALE) : "";
    ...
    public static String convertWGS84PolygonTo(final Context context, String polygon, int epsg) {

        try {
            final Database db = SpatialiteUtils.openSpatialiteDB(context);

            final Stmt stmt = db.prepare(String.format(Locale.US,"SELECT AsText(Transform(PolygonFromText('%s',4326), %d)) as result;",polygon,epsg));

            stmt.step();

            final String result = stmt.column_string(0);
